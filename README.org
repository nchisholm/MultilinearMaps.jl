...
- the dot product (~LinearAlgebra.dot~) of two ~StdUnitVector~​s amounts to checking if they point in the same direction; ~e ⋅ e′ := e.direction == e′.direction~
  - the dot product of a ~StdUnitVector~ with a "regular" vector just performs indexing; ~e ⋅ v := v[e.direction]~
...
@inline inds2uvecs(safety::S, inds::Vararg{Int}) where {S<:Safety} =
    map(i -> StdUnitVector{3}(i, safety), inds)
...
@btime(
    begin for l ∈ axes(A,4), k ∈ axes(A,3), j ∈ axes(A,2), i ∈ axes(A,1)
        @inbounds A[i,j,k,l] = _skew(StdUnitVector{3}(i, UNSAFE), StdUnitVector{3}(j, UNSAFE),
                                     StdUnitVector{3}(k, UNSAFE), StdUnitVector{3}(l, UNSAFE))
    end
    A
    end,
    setup=(A = MArray{NTuple{4,3}, Int64}(undef))
