...
#+begin_src jupyter-julia
materialize(Array{Float32}, skew);
#+end_src
...
let x⃗s = (SVector(x, y, 0.) for x ∈ LinRange(-3, 3, 201), y ∈ LinRange(-3, 3, 201))
    ...
    materialize(Point3f, stresslet(first(x⃗s))(:,n⃗,f⃗))[SOneTo(2)]

    # Makie needs a function in a rather speficic format.
    on_xy_plane(form, args) = x -> let x2 = SVector(x[1], x[2], 0.)
        materialize(Point3f, form(x2)(args...))[SOneTo(2)]
    end
...
*** Materialization of fully contracted case

#+begin_src jupyter-julia
let x = SVector{3,Float64}(1,2,3)
    e = SVector{3,Float64}(1,1,1)
    f = SVector{3,Float64}(3,2,1)
    stokeslet(x)(e,f)              # Produces a Float64 (not iterable)!
    # SArray(stokeslet(x)(e,f))      # so this doesn't work
    Scalar(stokeslet(x)(e,f))      # but this does.
end
#+end_src
...
let
    ...
    # f_refl = reflect(f)
    # ê = StdUnitVector{3}
    function stokeslet_nostress(x, y, f)::MultilinearMap
        y_img = reflect(y)
        stokeslet(x - y)(:, f) + stokeslet(x - reflect(y))(:, reflect(f))
    end

    velfield(x2) = let x⃗ = SVector(x2[1], x2[2], 0.)
        materialize(Point3f, stokeslet_nostress(x⃗, y⃗, f))[SOneTo(2)]
    end
