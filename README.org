...
Check that thing work correctly.
...
#+begin_src jupyter-julia
@btime [inner(ê{3}(i), ê{3}(j)) for i ∈ 1:3, j ∈ 1:3]
#+end_src
...
#+begin_src jupyter-julia :results scalar :eval no
...
Very odd quirk: if you change ~MultilinearMap{Tuple{}}~ to ~MultilinearForm{0,3} === MultilinearMap{Tuple{}}~, the /other/ benchmarks slow down.
...
@testset "Vector Space" begin
    ...
    @testset "Sums" begin
        ...
        @test_throws MethodError inner + skew
    end;
...
: Vector Space  | [32m   8  [39m[36m    8[39m
