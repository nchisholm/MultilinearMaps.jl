...
#+begin_src jupyter-julia
sphharm30(n̂) = MultilinearForm{0,3}(() -> 1.0)
...
sphharm33(n̂) = MultilinearForm{3,3}((v1, v2, v3) ->
    (n̂⋅v1)*(n̂⋅v2)*(n̂⋅v2) - ((v1⋅v2)*(n̂⋅v3) + (v3⋅v1)*(n̂⋅v2) + (v2⋅v3)*(n̂⋅v1))/5)
out = @btime SArray(sphharm33(x)) setup=(x=normalize(rand(SVector{3})))
#+end_src

These provide nice benchmarks
#+begin_src jupyter-julia
let n̂ = rand(SVector{3})
    (u, v, w) = ntuple(_ -> rand(SVector{3}), Val(3))
    println("Second order form")
    println("  all components -> 3x3 matrix")
    @btime SArray(sphharm32(n̂)) setup=(n̂=$n̂; u=$u; v=$v; w=$w)
    println("  single contraction -> length-3 vector  (matrix-vector prodct)")
    @btime SArray(sphharm32(n̂)(:, :)) setup=(n̂=$n̂; u=$u; v=$v; w=$w)
    println("  double contraction -> scalar  (quadratic form)")
    @btime sphharm32(n̂)(u, v) setup=(n̂=$n̂; u=$u; v=$v; w=$w)
    println()

    println("Thrid order form")
    println("  all components -> 3x3x3 array")
    @btime SArray(sphharm33(n̂)(:, :, :)) setup=(n̂=$n̂; u=$u; v=$v; w=$w)
    println("  single contraction -> 3x3 matrix")
    @btime SArray(sphharm33(n̂)(:, :, w)) setup=(n̂=$n̂; u=$u; v=$v; w=$w)
    println("  double contraction -> length-3 vector")
    @btime SArray(sphharm33(n̂)(:, v, w)) setup=(n̂=$n̂; u=$u; v=$v; w=$w)
    println("  full contraction -> scalar")
    @btime sphharm33(n̂)(u, v, w) setup=(n̂=$n̂; u=$u; v=$v; w=$w)
end;
#+end_src

** Finally, some Stokes-flow hydrodynamics
